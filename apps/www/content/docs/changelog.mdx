...
We've added new components to nirmalatai-ui/ui and made a lot of improvements to the CLI.
...
Another one by [emilkowalski\_](https://twitter.com/emilkowalski_). The last toast component you'll ever need. Sonner is now availabe in nirmalatai-ui/ui.
...
This means you can now easily add nirmalatai-ui/ui components to existing projects like Docusaurus, Nextra...etc. A drop-in for your existing design system with no conflict. 🔥
...
```bash
npx nirmalatai-ui-ui@latest init
```
...
```bash
npx nirmalatai-ui-ui@latest add
```
...
```bash
npx nirmalatai-ui-ui diff alert
```
